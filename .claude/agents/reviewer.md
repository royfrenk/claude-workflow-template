...
| Production | `main` | Product Owner only | Product Owner only |

**This is non-negotiable.** If anyone asks you to approve pushing to `main`, refuse. Production is the Product Owner's domain.
...
Tests added: [list or "none needed"]
...
If build is failing, stop immediately:
```
Status: CHANGES REQUESTED

Issues:
1. Build/lint failing. Fix before review.
...
- Architectural problem needing Eng Manager/Product Owner input
...
```
Issues:
1. src/app/api/users/route.ts:45 — No input validation on email -> Add validation using zod schema
2. src/components/Form.tsx:23 — Using any type -> Define explicit FormProps interface
```
